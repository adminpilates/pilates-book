...
export async function PUT(
  ...
  try {
    ...
    const sessionType = await prisma.pSessionType.update({
      where: { id },
      data: {
        name,
        description,
...
export async function DELETE(
  ...
  try {
    ...
    const sessionsCount = await prisma.pSession.count({
      where: { sessionTypeId: id, isActive: true },
    });

    if (sessionsCount > 0) {
      return NextResponse.json(
        { error: "Cannot delete session type with existing sessions" },
        { status: 400 }
      );
    }

    await prisma.pSessionType.update({
      where: { id },
      data: { isActive: false },
    });
